...
> {
  return invoke("get_app_edition_info_cmd");
}
...
export async function startUpdateDownload(downloadUrl?: string): Promise<void> {
  return invoke("start_update_download", { downloadUrl: downloadUrl ?? null });
}

export async function getUpdateDownloadProgress(): Promise<
  import("./types").DownloadProgress
> {
  return invoke("get_update_download_progress");
}
